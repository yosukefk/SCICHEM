...
IF( iflag /= 1 )THEN
  ...
  IF( ASSOCIATED(fldp%QLprof%QQ) )THEN            !Model QQ, etc.
...
!$OMP PARALLEL SHARED( grid,fld,fldp )
!$OMP DO PRIVATE(k,ip,i,pU,pV,U,V)
  DO k = 1,grid%nZ
...
!$OMP PARALLEL SHARED( grid,fld3d,fldp3d,UorV )
!$OMP DO PRIVATE(k,ip,i,fld,fldp)
DO k = 1,grid%nZ
