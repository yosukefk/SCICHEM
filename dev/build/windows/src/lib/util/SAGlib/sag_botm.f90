...
SAG_BottomMinMaxID = SAG_BottomFunctionID( grdI,SAG_MaxValue,nfld,jfld,restore,.TRUE. )
...
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(ix,iy,id,p0) IF(nx*ny > 1)
!$OMP SINGLE
DO ix = 1,nx
  DO iy = 1,ny
...
!$OMP TASK FIRSTPRIVATE(p0) DEFAULT(SHARED)
