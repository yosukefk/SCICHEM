...
IF( lAddMet )THEN
!------ Implicitly assuming this routine cannot be called in parallel
!       Otherwise SetMetGrid can cause conflicts
!$OMP CRITICAL (UpdateMetGrid)
  irv = SetMetGrid()
!$OMP END CRITICAL (UpdateMetGrid)
