...
IF( ASSOCIATED(wt)      )DEALLOCATE( wt,     STAT=alloc_stat )
IF( ASSOCIATED(gamT)    )DEALLOCATE( gamT,   STAT=alloc_stat )
...
ELSE
  ...
  irv = SAG_BottomFunctionID( grdI,SourceLocFunc,3,I_FIELD,.FALSE.,.FALSE. )
  IF( irv /= SAG_OK )THEN
    nError   = UK_ERROR
    eRoutine = 'SourceLocationSearch'
